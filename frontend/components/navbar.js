...
getCategorias().then(cats => {
  ...
  // ── Search desktop (el input está en index.html, no en el navbar) ──
  let allProducts = [];
  let searchTimer;

  getProductos().then(raw => { allProducts = raw.map(p => normalizarProducto(p)); });

  // Espera a que el DOM esté listo para conectar el buscador del HTML
  setTimeout(() => {
